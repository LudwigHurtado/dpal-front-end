...
import type { PlasticWatchStep } from '../types';

type Props = {
  open: boolean;
  steps: PlasticWatchStep[];
  logLines: string[];
  isRunning: boolean;
  onClose: () => void;
  onStop: () => void;
...
function statusLabel(status: PlasticWatchStep['status']): string {
...
function StepIcon({ status }: { status: PlasticWatchStep['status'] }) {
...
const PlasticWatchAutomationPanel: React.FC<Props> = ({
  open,
  steps,
  logLines,
  isRunning,
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end bg-slate-900/25 backdrop-blur-[1px]">
      <div
        ...
        role="dialog"
        aria-labelledby="plastic-watch-title"
      >
        <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-4 py-3">
          <div>
            <p id="plastic-watch-title" className="text-sm font-semibold text-slate-900 tracking-tight">
              Watch DPAL Work
            </p>
...
export default PlasticWatchAutomationPanel;
