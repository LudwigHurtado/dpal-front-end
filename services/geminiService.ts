...
export async function generateNftImage(hero: Hero, reportContext: any, prompt: string, theme?: NftTheme): Promise<string> {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
        ...
        contents: {
            parts: [{ text: `A futuristic holographic accountability artifact for a decentralized ledger. Theme: ${theme || 'Cyberpunk'}. Concept: ${prompt}. Cinematic lighting, 8k resolution, metadata tags visible.` }]
        },
        config: {
...
export async function generateHeroPersonaImage(prompt: string, arch: Archetype, sourceImageData?: string): Promise<string> {
    ...
    try {
        const ai = getAiClient();
        // Use gemini-2.5-flash-image directly for hero portraits to avoid non-existent backend dependencies
        const imageResponse = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: {
                parts: [
                    ...(sourceImageData ? [{ inlineData: { mimeType: 'image/jpeg', data: sourceImageData.split(',')[1] } }] : []),
                    { text: `A futuristic hero character portrait. Archetype: ${arch}. Description: ${prompt}. Cinematic lighting, detailed costume, digital oversight aura.` }
                ]
            },
            config: {
                imageConfig: { aspectRatio: "1:1" }
