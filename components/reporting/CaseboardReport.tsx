...
import { categoryToGatewayId } from '../sectors/categoryGatewayRegistry';
import type { CaseboardRoleRow } from './caseboardRegistry';
import { getCaseboardConfig } from './caseboardRegistry';
...
const BASE_CASEBOARD_IDS = [
  ...
  { id: 'where', title: 'Where', hint: '' },
  { id: 'when', title: 'When', hint: 'Date / time or timeframe' },
  { id: 'who', title: 'Who was affected', hint: '' },
...
const SCHOOL_CARD = {
  id: 'school' as const,
  title: 'School context (structured)',
  hint: 'Setting, admin notification, and response quality.',
};
...
export interface CaseboardReportProps {
  category: Category;
  addReport: (report: Omit<Report, 'id' | 'timestamp' | 'hash' | 'blockchainRef' | 'status'>) => void;
  prefilledDescription?: string;
}

const CaseboardReport: React.FC<CaseboardReportProps> = ({ category, addReport, prefilledDescription }) => {
  const config = useMemo(() => getCaseboardConfig(category), [category]);
  const draftKey = config ? `dpal-caseboard-draft-${config.slug}-v1` : '';
  const legacyEducationDraftKey = 'dpal-education-caseboard-draft-v1';

  const caseboardIds = useMemo(() => {
    if (!config) return [];
    const withHints = BASE_CASEBOARD_IDS.map((c) => {
      if (c.id === 'where') return { ...c, hint: config.whereHint };
      if (c.id === 'who') return { ...c, hint: config.whoHint };
      return c;
    });
    if (config.extraSection === 'school') {
      return [...withHints, SCHOOL_CARD];
    }
    return [...withHints];
  }, [config]);
  ...
  const [pathCode, setPathCode] = useState<string | null>(null);
  const [roleRow, setRoleRow] = useState<CaseboardRoleRow | null>(null);
  ...
  useEffect(() => {
    ...
    const tryLoad = (raw: string | null) => {
      ...
      try {
        ...
        if (d.pathCode) setPathCode(d.pathCode);
        if (d.pathEdu02) setPathCode(d.pathEdu02);
    ...
    try {
      if (tryLoad(localStorage.getItem(draftKey))) return;
      if (category === Category.Education) tryLoad(localStorage.getItem(legacyEducationDraftKey));
    } catch {
      /* ignore */
    }
  }, [draftKey, category]);
  ...
  useEffect(() => {
    if (!config) return;
    const payload = {
      categoryGatewayId: categoryToGatewayId(category),
      what,
      where,
      when,
      who,
  ...
  }, [config, draftKey, category, what, where, when, who, pathLabel, pathCode, phase, setting, adminNotified, adminResponse]);
  ...
  const cardComplete: Record<string, boolean> = useMemo(() => {
    ...
    if (config?.extraSection === 'school') {
      base.school = Boolean(setting && adminNotified && adminResponse);
    }
    return base;
  }, [what, where, when, who, attachments, patternNote, patternFreq, followUp, setting, adminNotified, adminResponse, config?.extraSection]);
  ...
  const submit = () => {
    if (!safetyConfirmed || !roleRow || !pathLabel || !config) return;
    setIsSubmitting(true);
    const desc = [what.trim(), chips.length ? `Tags: ${chips.join(', ')}` : '', followUp.trim() ? `Follow-up: ${followUp}` : '']
      .filter(Boolean)
      .join('\n\n');

    const slug = categoryToGatewayId(category);
    const titlePrefix = category === Category.Education ? 'Education' : slug.replace(/-/g, '_');

    const baseReport = {
      title: `${titlePrefix}_Case_${Date.now().toString().slice(-4)}`,
      description: desc || `${category} case report`,
      category,
    ...
    if (roleRow.kind === 'education') {
      ...
      void Promise.resolve(
        addReport({
          ...baseReport,
          structuredData: {
            caseboard_path_code: pathCode,
            caseboard_path_label: pathLabel,
            caseboard_role_ui: roleRow.ui,
            caseboard_role_badge: roleRow.badge,
            caseboard_role_code: roleRow.code,
            caseboard_mission_mode: missionMode,
            caseboard_when: when,
            caseboard_pattern_notes: patternNote,
            caseboard_chips: chips,
            case_strength_tier: tier.label,
            case_strength_score: strengthScore,
            badges_earned: badges,
            fidelity: strengthScore,
            safety_checked: safetyConfirmed,
            reporting_experience: 'caseboard_v1',
            category_gateway_id: slug,
          },
        })
      ).finally(() => setIsSubmitting(false));
    }
    ...
    try {
      ...
      if (category === Category.Education) localStorage.removeItem(legacyEducationDraftKey);
    } catch {
      /* ignore */
    }
  };
      ...
      <div className="mb-8 rounded-2xl bg-white border border-stone-200/80 shadow-sm p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          ...
          <p className="text-xs text-stone-400">
            {PROGRESS_LABELS[phaseIndex]} · step {phaseIndex + 1} / 5
          </p>
        </div>
      ...
      {phase === 'board' && (
        <section className="space-y-6 animate-fade-in">
          ...
          <div className="grid gap-4">
            {caseboardIds.map((card) => {
              const done = cardComplete[card.id];
              return (
                <div
                  key={card.id}
...
export default CaseboardReport;
